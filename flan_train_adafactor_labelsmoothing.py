...
def cmd_train(args):
    ...
    writer.add_scalar("Loss/Train", float("nan"), 0)  # bootstrap
    writer.flush()
    ...
    if args.get("tb_autostart", False):
        launch_tensorboard(
            logdir=runs_root,  # parent: sees all runs
    ...
    # ---------- OPTIM/SCHED ----------
    # Adafactor with external LR + scheduler
    optimizer = Adafactor(
        model.parameters(),
        lr=args["lr"],
        weight_decay=args["weight_decay"],
        relative_step=False,     # use absolute lr
        scale_parameter=False,   # no internal scaling of lr
        warmup_init=False,
    )
    ...
    LS = float(CONFIG["train"].get("label_smoothing", 0.0))

    # ---------- Loss helper ----------
    import torch.nn.functional as F
    def ce_loss_with_ls(logits, labels, ls: float):
        # logits: (B, T, V), labels: (B, T) with -100 ignored
        ...
        with torch.no_grad():
            for i, b in enumerate(dloader):
                ...
                with torch.autocast(device_type="cuda", dtype=amp_dtype, enabled=use_autocast):
                    ...
                    loss_val = ce_loss_with_ls(outputs.logits, b["labels"], LS)
                    total += loss_val.item()
                nb += 1
        model.train()
    ...
    for epoch in range(1, args["epochs"] + 1):
        ...
        for step, batch in tqdm(enumerate(train_loader), total=len(train_loader), desc=f"[Train] epoch {epoch}/{args['epochs']}"):
            ...
            with torch.autocast(device_type="cuda", dtype=amp_dtype, enabled=use_autocast):
                ...
                loss = ce_loss_with_ls(outputs.logits, batch["labels"], LS) / args["grad_accum"]
